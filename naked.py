...
if __name__ == "__main__":
	...
	# 9 - id
		...
		if len(ast_hazardous) > 0:	# are there any potentially hazardous asteroids?
			...
			ast_hazardous_by_time = sort_ast_by_time(ast_hazardous)
#			ast_hazardous.sort(key = lambda x: x[4], reverse=False)

			logger.info("Today's possible apocalypse (asteroid impact on earth) times:")
			for asteroid in ast_hazardous_by_time:
			...
			ast_hazardous_by_dist = sort_ast_by_pass_dist(ast_hazardous)
#			ast_hazardous.sort(key = lambda x: x[8], reverse=False)
			# print name, distance & NASA db link for the asteroid that will pass the closest to the Earth
			logger.info("Closest passing distance is for: " + str(ast_hazardous_by_dist[0][0]) + " at: " + str(int(ast_hazardous_by_dist[0][8])) + " km | more info: " + str(ast_hazardous_by_dist[0][1]))
		else:
